...
# zip up all the images in outputs and put it in the root
zipDreamstages = zipfile.ZipFile('dreams.zip', 'w')
for root, dirs, files in os.walk('outputs'):
  for file in files:
      zipDreamstages.write(os.path.join(root, file),file)
zipDreamstages.close()
